...
expName = 'expDesign'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Alex\\Documents\\MATLAB\\cmu\\stim_v2\\expDesign_lastrun.py',
...
for thisBlock in block:
    ...
    while continueRoutine:
        ...
        if blockStartText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        waitOnFlip = False
        if blockStartKey.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    for thisTrial in trials:
        ...
        soundStim.setVolume(1.0, log=False)
        trialKey.keys = []
        trialKey.rt = []
        _trialKey_allKeys = []
        # keep track of which components have finished
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if fixDotBlue.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixDotBlue.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if trialKey.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trialKey.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
