...
currentStim = int(input('Current Stim: '))


# Store info about the experiment session
psychopyVersion = '2021.2.3'
expName = 'expDesign_'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Alex\\Documents\\MATLAB\\cmu\\stim_v2\\expDesign_.py',
...
for thisBlock in block:
    ...
    if block.thisRepN + 1 == blockNum:
        ...
        while continueRoutine:
            ...
            pathTrialInfo = os.path.join(os.getcwd(), 'trialInfo', str(subID), str(block.thisRepN + 1) + '.csv')
            ...
            if frameN == 0:
                ...
                loadStim(iStimSide, iStim, currentStim, stimOutlet)
        ...
        for thisTrial in trials:
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if trialKey.status == STARTED and not waitOnFlip:
                    theseKeys = trialKey.getKeys(keyList=['num_1', 'num_2', 'num_3'], waitRelease=False)
            ...
            # Display response
            tempStr = ''
            if trialKey.keys is not None:
                if len(trialKey.keys) >= 3:
                    nKey = 3
                else:
                    nKey = len(trialKey.keys) 

                for i in range(nKey):
                    tempStr = tempStr + trialKey.keys[i][-1]

                if tempStr == str(sylTarg):
                    nCorrect += 1

            print('Response: {} | {} | {}/{}'.format(tempStr, tempStr == str(sylTarg), nCorrect, len(trials.trialList)))
            print()
